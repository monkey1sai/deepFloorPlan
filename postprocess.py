...
from utils import rgb_ind_convertor as rgbi
...
def post_process(input_dir, save_dir, merge=True):
	...
	for i in xrange(n):
		...
		im_ind = rgbi.rgb2ind(im, color_map=rgbi.floorplan_fuse_map)
		...
		cw_mask = util.fill_break_line(cw_mask)
			
		fuse_mask = cw_mask + rm_mask
		fuse_mask[fuse_mask>=1] = 255

		# refine fuse mask by filling the hole
		fuse_mask = util.flood_fill(fuse_mask)
		fuse_mask = fuse_mask // 255	

		# one room one label
		new_rm_ind = util.refine_room_region(cw_mask, rm_ind)
		...
		if merge:
			...
			rgb = rgbi.ind2rgb(new_rm_ind, color_map=rgbi.floorplan_fuse_map)
		else:
			rgb = rgbi.ind2rgb(new_rm_ind)
		image.imsave(out_paths[i], rgb)
